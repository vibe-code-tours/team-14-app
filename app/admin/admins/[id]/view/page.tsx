  ...
  if (error || !admin) {
    return (
      <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-100 text-center">
        <p className="text-slate-500">{error || "Admin not found"}</p>
        <Link
          href="/admin/admins"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 animate-fade-in" style={{ animationDelay: "0ms" }}>
        <div>
          <h1 className="text-2xl font-bold text-slate-800">🛡️ View Admin</h1>
          <p className="text-slate-500 text-sm">{admin.nickname || admin.fullName}</p>
        </div>
        <Link
          href="/admin/admins"
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 animate-fade-in" style={{ animationDelay: "150ms" }}>
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-800">{admin.fullName}</h3>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-800 bg-slate-50 p-3 rounded-lg">{admin.fullName}</p>
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Nickname</label>
            <p className="mt-1 text-sm text-slate-800 bg-slate-50 p-3 rounded-lg">{admin.nickname || "—"}</p>
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Email</label>
            <p className="mt-1 text-sm text-slate-800 bg-slate-50 p-3 rounded-lg">{admin.email}</p>
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Role</label>
            <p className="mt-1">
              <span className="text-xs font-semibold px-3 py-1.5 rounded-full bg-emerald-100 text-emerald-800 capitalize">
                {admin.role}
              </span>
            </p>
          </div>

...
export default function AdminReviewsPage() {
  ...
  const [confirmModal, setConfirmModal] = useState<{
    ...
    reviewName: string;
    isVisible: boolean;
  }>({ isOpen: false, reviewId: 0, reviewName: "", isVisible: true });
  ...
  const handleRequestToggle = (id: number, isVisible: boolean, name: string) => {
    setConfirmModal({
      isOpen: true,
      reviewId: id,
      reviewName: name,
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">💬 Reviews</h1>
      ...
      {confirmModal.isOpen && createPortal(
        <div className="fixed z-50 top-24 left-1/2 -translate-x-1/2 w-full max-w-sm">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl overflow-hidden mx-4">
            <div className={`px-3 py-2 ${confirmModal.isVisible ? "bg-linear-to-r from-amber-500 to-orange-500" : "bg-linear-to-r from-emerald-500 to-teal-500"}`}>
              <div className="flex items-center gap-2">
                <span className="text-lg">
                  {confirmModal.isVisible ? "⚠️" : "✅"}
                </span>
                <h3 className="text-sm font-bold text-white">
                  {confirmModal.isVisible ? "Hide Review" : "Show Review"}
                </h3>
              </div>
            </div>
            <div className="px-3 py-2">
              <p className="text-slate-600 dark:text-slate-300 text-xs leading-relaxed">
                {confirmModal.isVisible
                  ? `Are you sure you want to hide the review from "${confirmModal.reviewName}"?`
                  : `Are you sure you want to show the review from "${confirmModal.reviewName}"?`}
              </p>
            </div>
            <div className="px-3 pb-3 flex gap-2 justify-end">
              ...
              >
                {confirmModal.isVisible ? "Hide" : "Show"}
              </button>
            </div>
          </div>
        </div>,

  ...
  if (error || !user) {
    return (
      <div className="bg-white dark:bg-slate-800 p-8 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 text-center">
        <p className="text-slate-500 dark:text-slate-400">{error || "User not found"}</p>
        <Link
          href="/admin/users"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 animate-fade-in" style={{ animationDelay: "0ms" }}>
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">👤 View User</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm">{user.nickname || user.fullName}</p>
        </div>
        <Link
          href="/admin/users"
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 animate-fade-in" style={{ animationDelay: "150ms" }}>
        <div className="flex items-center gap-4 mb-6">
          <div
            className={`w-16 h-16 rounded-full overflow-hidden bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-400 dark:text-slate-500 ${user.image ? "cursor-pointer hover:ring-2 hover:ring-emerald-500 transition" : ""}`}
            onClick={() => user.image && setShowLightbox(true)}
          >
            {user.image ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-700 p-3 rounded-lg">{user.fullName}</p>
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Nickname</label>
            <p className="mt-1 text-sm text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-700 p-3 rounded-lg">{user.nickname || "—"}</p>
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Email</label>
            <p className="mt-1 text-sm text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-700 p-3 rounded-lg">{user.email}</p>
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Role</label>
            <p className="mt-1 text-sm text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-700 p-3 rounded-lg capitalize">{user.role}</p>
          </div>
        </div>
      </div>
      ...
      {/* Image Lightbox */}
      {showLightbox && user.image && createPortal(
        <div

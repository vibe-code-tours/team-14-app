...
const SearchWithDropdown = () => {
  ...
  const [selectedRegion, setSelectedRegion] = useState('กทม และภาคกลาง');
  const dropdownRef = useRef<HTMLDivElement>(null);

  // Static list of regions based on project scripts/docs
  const regions = [
    'กทม และภาคกลาง',
    'ภาคเหนือ',
    ...
    <div className="w-full max-w-sm min-w-[300px]">
      <div className="relative mt-2" ref={dropdownRef}>
        <div className="absolute top-1 left-1 flex items-center">
          ...
          >
            <span className="text-ellipsis overflow-hidden w-32 text-left">{selectedRegion}</span>
          ...
          {isOpen && (
            <div className="min-w-[200px] overflow-hidden absolute left-0 w-full mt-10 top-0 bg-white border border-slate-200 rounded-md shadow-lg z-10">
              <ul>
                {regions.map((region) => (
                  <li
                    key={region}
        ...
        <input
          type="text"
          className="w-full bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded-md pr-12 pl-40 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-sm focus:shadow"
